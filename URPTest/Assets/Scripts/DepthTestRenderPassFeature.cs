...
public class DepthTestRenderPassFeature : ScriptableRendererFeature
{
    ...
    DepthTestRenderPass m_ScriptablePass;

    public override void Create()
    {
        int passIndex = settings.mMat != null ? settings.mMat.passCount - 1 : 1;
        settings.blitMaterialPassIndex = Mathf.Clamp(settings.blitMaterialPassIndex, -1, passIndex);
        m_ScriptablePass = new DepthTestRenderPass("DepthTestRenderPass", settings.renderPassEvent, settings.mMat, settings.contrast);
